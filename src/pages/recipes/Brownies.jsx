...
const Brownies = () => {
  const recipeData = {
    ...
    baseServings: 9, // Nombre de parts de base
  };
  ...
  // 🧮 FONCTION DE CALCUL RÉPARÉE
  const calculateQuantity = (amount, base) => {
    if (!amount) return ""; // Si pas de quantité, on renvoie vide
    
    // On s'assure que c'est bien un chiffre
    const numAmount = parseFloat(amount);
    if (isNaN(numAmount)) return amount; // Si c'est du texte (ex: "QS"), on renvoie le texte

    // Le produit en croix
    const scaled = (numAmount * servings) / base;

    // Formatage propre : pas de virgule si c'est un entier, sinon 1 chiffre après la virgule
    return Number.isInteger(scaled) ? scaled : scaled.toFixed(1).replace('.0', '');
  };

  // 👇 LISTE D'INGRÉDIENTS STANDARDISÉE (Amount = Chiffre, Unit = Texte)
  const ingredients = [
  ...
  const structuredData = {
    ...
    "author": { "@type": "Person", "name": "Maison Dorée" },
    "prepTime": "PT15M",
    "cookTime": "PT25M",

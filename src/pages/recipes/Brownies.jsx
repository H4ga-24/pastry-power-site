...
const Brownies = () => {
  ...
  // Fonction de calcul mise à jour pour le nouveau format
  const calculateAmount = (amount, base) => {
    if (!amount || isNaN(amount)) return amount; // Si c'est "1 pincée" (texte), on renvoie tel quel
    const scaled = (amount * servings / base);
    // On arrondit pour éviter les 33.3333g
    return Number.isInteger(scaled) ? scaled : scaled.toFixed(1).replace('.0', '');
  };

  // 👇 LISTE CORRIGÉE POUR LE MODE CUISINE
  // Important : Sépare bien 'amount' (chiffre) et 'unit' (texte)
  const ingredients = [
    { name: "Chocolat noir 70%", amount: 200, unit: "g" },
    { name: "Beurre", amount: 150, unit: "g" },
    { name: "Oeufs", amount: 3, unit: "pcs" }, // Mets "pcs" ou "" pour les unités
    { name: "Sucre semoule", amount: 150, unit: "g" },
    { name: "Farine", amount: 60, unit: "g" },
  ...
  const structuredData = {
    ...
    "recipeIngredient": ingredients.map(ing => `${calculateAmount(ing.amount, recipeData.baseServings)} ${ing.unit} ${ing.name}`),
                  ...
                  {ingredients.map((ing, i) => (
                    <li key={i} className="flex justify-between items-center text-sm pb-2 border-b border-white/5 last:border-0">
                      ...
                      <span className="text-[#D4AF37] font-medium">
                        {calculateAmount(ing.amount, recipeData.baseServings)} {ing.unit}
                      </span>
                    </li>
                  ))}
                </ul>
                    ...
                    <div className="flex items-start gap-4 animate-fade-in">
                      ...
                      <div className="space-y-4">
                        ...
                        <ul className="space-y-3 text-gray-400 leading-relaxed">
                           ...
                           <li>Variante : Remplacez les noix de pécan par des noix de macadamia.</li>
                        </ul>
                      </div>
                    </div>
                  </TabsContent>
                    ...
                    <div className="flex items-start gap-4 animate-fade-in">
                      ...
                      <div className="space-y-4">
                        ...
                        <p className="text-gray-400 leading-relaxed italic">
                          "Le brownie parfait se joue à la minute près. Si vous le cuisez trop, il devient un gâteau au chocolat classique."
                        </p>
                      </div>
                    </div>
                  </TabsContent>
                    ...
                    <div className="flex items-start gap-4 animate-fade-in">
                      ...
                      <div className="space-y-4">
                        ...
                        <ul className="grid grid-cols-2 gap-2 text-gray-400">
                          <li className="flex items-center gap-2"><div className="w-1.5 h-1.5 bg-[#D4AF37] rounded-full"></div>Cadre carré</li>

...
// Globs SANS eager: true — Vite génère uniquement une map { path → loader }
// Les fichiers ne sont téléchargés qu'à la demande, quand on appelle loader().
...
const DynamicPage = () => {
  ...
  useEffect(() => {
    ...
    // 1. Trouver l'entrée dans le catalogue (recherche O(n) légère sur métadonnées)
    ...
    // 2. Trouver la bonne loader function dans le glob map
    ...
    // 3. Charger uniquement CE fichier à la demande
    loaders[loaderKey]()
      .then(mod => {
  ...
  if (!entry.isTech) {
    // Composant sur-mesure (ex : Mousses, Pâte de fruits)
